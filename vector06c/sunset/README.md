...
Файл `.rom` понимают все эмуляторы «Вектора»: v06x, Emu80, эмулятор Дмитрия Целикова.
Например:
...
Растр защёлкивает запись только в бордюре, поэтому вся палитра перезаписывается сразу после кадрового прерывания процедурой `colorset` — она целиком взята из [FastPalProg 2.1](../vector06c/fast_pal_prog) Ивана Городецкого (2018), где расстановка команд и задержек уже выверена по тактам.
...
Сборка v06x на современной системе требует нескольких правок: `boost::asio::io_service` переименован в `io_context`, в `memory.cpp` и `debug.cpp` не хватает `#include <algorithm>`, а `debug.cpp`, `fsimage.cpp` и `utils_string.cpp` не перечислены в `CMakeLists.txt`.
