...
Скрипт рисует все кадры, сочиняет таблицы музыки, собирает `nyancat.asm` своим ассемблером, упаковывает образ и приклеивает к нему распаковщик.

Самотесты:

```
python3 test_asm8080.py   # ассемблер: 244 опкода 8080 плюс 21 проверка директив
...
| `asm8080.py` | двухпроходный ассемблер i8080 (мнемоника Intel, `REPT/ENDR`, `$`) |
| `test_asm8080.py` | сверка всех 244 документированных опкодов с эталонной таблицей |
...
`build.py` проверяет это утверждением на сборке.
...
* `test_asm8080.py` - все 244 документированных опкода 8080 против эталонной таблицы, 15 проверок директив, `$` и `REPT` и 6 негативных проверок.
